...
class FixedBallotCostMDP(object):
    ...
    def determineBVFunction(self,referenceDifficultyDistribution):
        ...
        if os.path.exists('log/bvFunctions/%s|%1.2f_fixed' % (self.systemParameters.stringifyForBVFunction(referenceDifficultyDistribution),self.systemParameters.threshold)):
            print "Reading BV function."
            bvFunction = pickle.load(open('log/bvFunctions/%s|%1.2f_fixed' % (self.systemParameters.stringifyForBVFunction(referenceDifficultyDistribution),self.systemParameters.threshold)))
            os.chdir(SystemParameters.path)
            if not os.path.exists('Plots/BVFunction/%s|%1.2f_fixed.eps' % (self.systemParameters.stringifyForBVFunction(referenceDifficultyDistribution),self.systemParameters.threshold)):
                for cost,func in bvFunction.items():
                    plt.plot(2*np.linspace(0.5,1.0,self.systemParameters.bvFunctionGranularity + 1) - 1,func,label='Price = %d' % (cost))
                plt.legend(loc='best')
                plt.savefig('Plots/BVFunction/%s|%1.2f_fixed.eps' % (self.systemParameters.stringifyForBVFunction(referenceDifficultyDistribution),self.systemParameters.threshold), format='eps', dpi=1000)
            return bvFunction
        ...
        pickle.dump(bvFunction,open('log/bvFunctions/%s|%1.2f_fixed' % (self.systemParameters.stringifyForBVFunction(referenceDifficultyDistribution),self.systemParameters.threshold),'w'))
        return bvFunction
    ...
    def determineAdmissibleBallotsToCompletion(self):
        ...
        os.chdir(SystemParameters.path + 'TMDP')
        if os.path.exists('log/admissibleStates/%s|%1.2f_fixed' % (self.systemParameters.stringifyForTransitionTable(),self.systemParameters.threshold)):
            print "Reading admissible states."
            validE_b = pickle.load(open('log/admissibleStates/%s|%1.2f_fixed' % (self.systemParameters.stringifyForTransitionTable(),self.systemParameters.threshold)))
            return validE_b
        ...
        pickle.dump(validE_b,open('log/admissibleStates/%s|%1.2f_fixed' % (self.systemParameters.stringifyForTransitionTable(),self.systemParameters.threshold),'w'))
        return validE_b
    ...
    def createValidStateTransitionTableUpdated(self,simulations=2):
        os.chdir(SystemParameters.path + 'TMDP')
        if os.path.exists('log/transitionTables/%s|%1.2f_fixed' % (self.systemParameters.stringifyForTransitionTable(),self.systemParameters.threshold)):
            while True:
                try:
                    os.mkdir('locks/%s|%1.2f_fixed' % (self.systemParameters.stringifyForTransitionTable(),self.systemParameters.threshold))
                    break
                except OSError:
                    pass
            print "Reading valid state transition table."
            (validSameCostTransitions,validCrossCostTransitions) = pickle.load(open('log/transitionTables/%s|%1.2f_fixed' % (self.systemParameters.stringifyForTransitionTable(),self.systemParameters.threshold)))
            os.rmdir('locks/%s|%1.2f_fixed' % (self.systemParameters.stringifyForTransitionTable(),self.systemParameters.threshold))
        ...
        os.chdir(SystemParameters.path + 'QualityPOMDP')
        qualityPOMDPPolicyList = {x:FixedPolicy(self.systemParameters,self.systemParameters.threshold) for x in self.possibleCosts}
        tasks = []
        POMDPLookup = {}
                            ...
                            if not E_b1 == 0:
                                ...
                                for _ in xrange(simulations):
                                    ...
                                    for ballot in ballots:
                                        ...
                                        controller.addAvailableQuestion(POMDPLookup[questionID])
        ...
        os.chdir(SystemParameters.path + 'TMDP')
        pickle.dump((validSameCostTransitions,validCrossCostTransitions),open('log/transitionTables/%s|%1.2f_fixed' % (self.systemParameters.stringifyForTransitionTable(),self.systemParameters.threshold),'w'))
        return (validSameCostTransitions,validCrossCostTransitions)
